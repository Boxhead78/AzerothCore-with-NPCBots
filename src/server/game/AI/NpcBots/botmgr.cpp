...
void BotMgr::LoadConfig(bool reload)
{
    ...
    _enableclass_wander_sphynx      = false; // sConfigMgr->GetBoolDefault("NpcBot.WanderingBots.Classes.ObsidianDestroyer.Enable", true);
    _enableclass_wander_archmage    = false; // sConfigMgr->GetBoolDefault("NpcBot.WanderingBots.Classes.Archmage.Enable", true);
    _enableclass_wander_dreadlord   = false; // sConfigMgr->GetBoolDefault("NpcBot.WanderingBots.Classes.Dreadlord.Enable", true);
    _enableclass_wander_spellbreaker= false; // sConfigMgr->GetBoolDefault("NpcBot.WanderingBots.Classes.SpellBreaker.Enable", true);
    _enableclass_wander_darkranger  = false; // sConfigMgr->GetBoolDefault("NpcBot.WanderingBots.Classes.DarkRanger.Enable", true);
    _enableclass_wander_necromancer = false; // sConfigMgr->GetBoolDefault("NpcBot.WanderingBots.Classes.Necromancer.Enable", true);
    _enableclass_wander_seawitch    = false; // sConfigMgr->GetBoolDefault("NpcBot.WanderingBots.Classes.SeaWitch.Enable", true);
    _enableclass_wander_cryptlord   = false; // sConfigMgr->GetBoolDefault("NpcBot.WanderingBots.Classes.CryptLord.Enable", true);
...
void BotMgr::Update(uint32 diff)
{
    ...
    for (BotMap::const_iterator itr = _bots.begin(); itr != _bots.end(); ++itr)
    {
        ...
        else if (BotMgr::IsRaidReviveActive())
        {
            ...
            if (_owner->IsAlive())
            {
                if (bot->IsInMap(_owner) && !bot->IsAlive() &&
                    !_owner->IsBeingTeleported() && !_owner->GetMap()->IsBattleArena() && !_owner->IsInFlight() &&
                    !_owner->HasUnitFlag2(UNIT_FLAG2_FEIGN_DEATH) && !_owner->HasInvisibilityAura() && !_owner->HasStealthAura())
                {
                    _reviveBot(bot);
