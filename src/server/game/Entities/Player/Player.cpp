...
uint32 Player::DurabilityRepair(uint16 pos, bool cost, float discountMod, bool guildBank)
{
    ...
    if (cost)
    {
        ...
        if (LostDurability > 0)
        {
            ...
            if (HasAura(98593)) // Best Deals anywhere
                costs *= 0.9;
...
void Player::CheckAreaExploreAndOutdoor()
{
    ...
    if (!areaEntry)
    {
        LOG_ERROR("entities.player", "Player '{}' ({}) discovered unknown area (x: {} y: {} z: {} map: {} areaid: {})",
                       GetName(), GetGUID().ToString(), GetPositionX(), GetPositionY(), GetPositionZ(), GetMapId(), areaId);
        return;
    }
...
void Player::SetRestBonus(float restBonusNew)
{
    ...
    float restBonusMax = (float)GetUInt32Value(PLAYER_NEXT_LEVEL_XP) * restBonusMultiplier / 4;
...
bool Player::canFlyInZone(uint32 mapid, uint32 zone, SpellInfo const* bySpell)
{
    ...
    if ((v_map == 0 || v_map == 1))
    {
        if (!HasSpell(98576)) // 98576 = Flying License
        {
            return false;
        }
    }
